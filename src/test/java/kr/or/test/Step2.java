...
class MemberService {//이 서비스 클래스에서는 회원출력에 관련된 메서드를 모아 놓습니다.(아래)
	public void printMember(MemberVO[] members) {//members는 레코드가 여러개=여러줄
		...
		int cnt=0;
		for(MemberVO member:members) {//members여러 레코드중 1개의 레코드를 member오브젝트객체로 대체
			cnt = cnt + 1;
...
public class Step2 {
	...
	public static void main(String[] args) {
		// 실행 메서드
		MemberVO memberVO = new MemberVO();//자바에서 오브젝트 생성하는 방법
		...
		//위 각각  레코드별로 3개의 레코드를 1군데 담을 배열 클래스오브젝트를 하나 생성
