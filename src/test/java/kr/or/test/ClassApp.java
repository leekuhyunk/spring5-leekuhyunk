...
public class ClassApp {
	...
	public static void main(String[] args) {
		...
		graphicObject.draw();//클래스형 변수명을 지정할떄, 카멜표기(낙타등)를 적용.
		//더하기, 빼기 int sum = 5 + 3
 	}
...
abstract class GraphicObject {
	...
	//다른 해석하면, 목차만 있고, 내용이 없는 구조. 내용은 별도의 클래스에 있음.
}

class Triangle extends GraphicObject {
	//메서드 오버라이드(메서드이름이 같은 것을 상속관계에서 실행)=부모 메서드 재정의, 대표적인 다형성을 구현한다.
	@Override//메서드 재정의
	void draw() {
		System.out.println("  *");
		System.out.println(" * * ");
...
class Circle {
	private int r;//멤버변수 반지름  이글래스에서 전역변수
	public Circle(int a) {//생성자매서드 a인자=매개변수=파라미터 값은 new키워드로 오브젝트 만들어질때 발생
		r = a;
	}
	public double getleekuhyunk() {
		return r*r*3.14;//원의 넓의를 반환
	}
}
