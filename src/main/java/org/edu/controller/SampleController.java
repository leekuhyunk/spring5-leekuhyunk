...
public class SampleController {
	/**
	 * 루트 / 라는 이름으로 쵸정 받으면, home.jsp파일에 화면출력 (렌더링)이 됩니다.(아래)
	 * HomeController.java 를 저장하면, 이클립스에서는 HomeController.class 실행 가능한 파일을 생성
	...
	public String contact_send() {
		//데이터 전송후에 페이지 이동이 필요합니다. 새로고침=자동등록을 방지하기 위해서(게시판테러방지용)
	...
	public String index() {//메서드(함수)구성: 리턴(반환값출력)형태지정 메서드명(매개변수) {구현내용} String index (){}
		...
		//메서드명 제일 앞에있는 public는 접근권한에 대한키워드인데요, public(공공-다른 클래스에서 접근가능), private(내부)
		//, private(내부-현재HomeController클래스에서만 접근가능)
		// public String index() { } 자바(스프링) 메서드 기본형태
		// ${document).ready(function() { }); j쿼리 기본형태
		//리턴반환값으로 사용한 String 클래스는 문자열을 반환되고, 여기서는 index.jsp와 매칭되는 문자열을 반환.
		//여기서는 index.jsp와 매칭되는 문자열을 반환. 상대경로가 필요합니다.
		//스프링MVC프로젝트에서 상대경로의 최상위는 views폴더 최상위 root폴더 입니다.
		// 접두사(prefix), 접미사(suffix) index(접두사-파일명).jsp(접미사-확장자)
		return "sample/index";//접두사만 봔환값으로 넣습니다. 상대경로.
	}
}
