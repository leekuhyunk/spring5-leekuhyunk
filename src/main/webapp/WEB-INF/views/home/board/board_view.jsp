...
$(document).ready(function() {
	$("#insertReplyBtn").on("click", function() {//댓글등록버튼을 클릭했을 때 구현내용(아래)
		if("${session_enabled}" == "") {//버튼클릭시 비로그인시 로그인 화면으로 유도
			...
			location.href = "/login";//자바스크립트 내장기능(href:hyterTextReference:URL이동함수)
			return false;
		}
		//alert("디버그");
		...
		if(reply_text=="" || replyer==""){
			alert("댓글내용, 작성자는 필수 입력 사항 입니다.");
			return false;
		}
		$.ajax({//통신프로그램: J쿼리에서 내장된 함수ajax({}); 비동기통신특징(HTTP웹페이지의 단점을 해소 Ajax)
		...
		//서버(RestAPI서버=컨트롤러)-클라이언트(PC브라우저ajax=jsp단-화면)
			...
			data:JSON.stringify({
				...
				reply_text:reply_text,
				replyer:replyer
			}),//RestAPI서버로 보내는 Json값
			success:function(result) {//응답이 성공하면(상태값200)위경로에서 반환받은 result(json데이터)를 이용해서 화면을 재구현
				var reply_count = $("#reply_count").text();//겟Get
				if(reply_count == ""){reply_count=0;}//DB 오라클 초기값이 null일때 체크
				$("#reply_count").text(parseInt(reply_count)+1);//셋Set
				//댓글 3페이지를 보고 있다가, 댓글 입력했어요, 본인 작성한 댓글 바로 확인 가능하도록 1apge로 가도록 유도
