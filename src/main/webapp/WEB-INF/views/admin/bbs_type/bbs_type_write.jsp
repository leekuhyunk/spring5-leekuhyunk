...
<!-- 이렇게 상단부터 프로그램 내용을 해석-컴파일하는 과정을 인터프리터형 프로그램 언어입니다. -->
<script>
jQuery(document).ready(function(){
	jQuery("#board_type").bind("blur",function() {
		if(jQuery(this).val() != ""){
			//자바오브젝트의 board_type 값과 jQuery(this).val()
			...
			if(jQuery.inArray(jQuery(this).val(),board_type_array) >= 0) {//자바변수값과, input 게시판타입값 비교하기
				//inArray함수는 반환값이 인덱스값을 반환합니다. 0보다 크면 인덱스가 존재
				alert("기존 게시판이 존재 합니다. 다시 입력해 주세요");
				jQqery("#btn_insert").attr("disabled",true);
			}else{
